...
extern "C" void* ThreadDumper(void*) {
  ...
  do {
    ...
    if (count < 5) count++;
    {
		std::map<string, int> versionCount;
	  ...
	  for (vector<CAddrReport>::const_iterator it = v.begin(); it < v.end(); it++) {
		  ...
		  stat[3] += rep.uptime[3];
		  stat[4] += rep.uptime[4];
		  if (versionCount.count(rep.clientSubVersion)) {
			  std::map<string, int>::iterator it2 = versionCount.find(rep.clientSubVersion);
			  (*it2).second += 1;
		  }
		  else {
			  versionCount[rep.clientSubVersion] = 1;
		  }
      }
      fclose(d);
	  ...
	  FILE *fff = fopen("/usr/local/share/dnsseed/wallets.json", "w");
	  fprintf(fff, "{");
	  std::map<string, int>::iterator it2 = versionCount.begin();
	  while (it2 != versionCount.end()) {
			
		  fprintf(fff, "\"%s\":%i", (*it2).first.c_str(), (*it2).second);

		  it2++;
		  if(it2 != versionCount.end()) fprintf(fff, ",");
	  }
	  fprintf(fff, "}");
	  fclose(fff);
    }
...
static const string mainnet_seeds[] = {"78.8.60.138", "35.182.174.145", "104.131.187.251", "107.178.111.14", "127.0.0.1", ""};
static const string testnet_seeds[] = {"", "", ""};
static const string *seeds = mainnet_seeds;

extern "C" void* ThreadSeeder(void*) {
  if (!fTestNet){
    db.Add(CService("seed.dns.socialsend.io", 50050), true);
  }
  do {
